...
export default function AddLessonModal({ teacherId, onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.topic.trim()) { setError('Le theme du cours est obligatoire.'); return }
    setSubmitting(true)
    try {
      const lesson = await createLesson(teacherId, {
      ...
      setError(err.message ?? 'Impossible de creer le cours.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg glass-panel rounded-2xl p-6 shadow-2xl border border-border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-muted-foreground mb-1.5">Duree (minutes)</label>
